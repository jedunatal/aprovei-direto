    ...
    return (
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {/* Disciplina */}
                <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Disciplina</label>
                    <select
                        value={filters.discipline_id || ''}
                        onChange={(e) => handleDisciplineChange(e.target.value ? Number(e.target.value) : undefined)}
                        className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600"
                    >
                ...
                {/* Tópico / Assunto */}
                <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Assunto / Tópico</label>
                    <select
                        ...
                        className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 disabled:opacity-50"
                    >
                ...
                {/* Banca Examinadora */}
                <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Banca Examinadora</label>
                    <select
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-xs text-slate-700 font-medium">Apenas questões não resolvidas</span>
                </label>
            </div>
        </div>
    );
